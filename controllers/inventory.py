...
class InventoryController(http.Controller):
    ...
    @http.route('/cbm/inventory/search_barcode', type='json', auth='user')
    def search_barcode(self, barcode, location_id):
        """Search product by exact barcode match.

        If barcode is GS1-128 encoded (contains lot/expiry data),
        parses and returns lot_id, lot_name, expiry_date.

        Args:
            barcode: Barcode string
            location_id: stock.location ID for quantity lookup
        ...
        Returns:
            dict: {
                ...
                lot_id: int or False,
                lot_name: str,
                expiry_date: str or False (YYYY-MM-DD),
            }
        """
        try:
            Product = request.env['product.product']
